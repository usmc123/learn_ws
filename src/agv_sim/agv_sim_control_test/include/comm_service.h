...
namespace dst_ccms_api
{
    ...
    ** Output: True or False (CRC Verify Result) 
    这个函数是验证接收数据的CRC
    */ 
    unsigned int Verify_CRC8_Check_Sum(unsigned char *pchMessage, unsigned int dwLength);//这里输入的长度是包括CRC校验码的长度
    /* 
...
将数据设置CRC校验码插入到数据的尾部
*/ 
void Append_CRC8_Check_Sum(unsigned char *pchMessage, unsigned int dwLength); //这里输入的长度是包括CRC校验码的长度
/* 
